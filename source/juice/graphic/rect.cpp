...
namespace juice {
    ...
    Rect::Rect(float x, float y, float w, float h, glm::vec3 color)
    ...
    void Rect::SetRotation(float _r)
    {
        r = _r;
        CalculateModelViewMatrix();
    }
    
    void Rect::move(const glm::vec2& position)
    {
        x += position.x;
        y += position.y;
        CalculateModelViewMatrix();
    ...
    void Rect::Draw(const ref<Shader>& shader)
    {
